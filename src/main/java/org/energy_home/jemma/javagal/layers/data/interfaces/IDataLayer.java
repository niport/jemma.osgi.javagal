...
public interface IDataLayer extends IFrameCallback {
	...
	/**
	 * Starts a service discovery function. The StartServiceDiscovery procedure is
	 * invoked by an IPHA to perform a service discovery on the network to know
	 * which services could be offered by the devices in the ZigBee net.
	 * 
	 * @param timeout
	 *          the desired timeout value.
	 * @param aoi
	 ...
	 *          the address of interest for which the Node Descriptor is required.
	...
	/**
	 * Starts the Gateway Device synchronously. You can control the start behavior
	 * with the {@link StartupAttributeInfo} object.
	 * 
	 * @param timeout
	 *          the desired timeout value.
	 * @param sai
	...
	/**
	 * Configures an end point synchronously. Configuration values are carried by
	 * a {@link SimpleDescriptor} object.
	 * <p>
	 * The ConfigureEndpoint procedure is invoked by an IPHA in order to include
	 * an endpoint in the list of active endpoints and to set a simple descriptor
	 * for this endpoint in the ZDO of the ZGD. Consequently, if a ZigBee node
	 * issues a request to discover the active endpoints on the ZGD, the ZGD will
	 * indicate that the list of active endpoints includes the endpoint supplied
	 * in parameter of this procedure. If a ZigBee node issues a request to
	 * discover the simple descriptor supported by this endpoint on the ZGD, the
	 * ZGD will respond with the simple descriptor supplied in parameter of this
	 * procedure. An IPHA normally call such procedure when it implements a ZigBee
	 * application object on behalf of the ZigBee node of the ZGD so that from a
	 * node in the ZigBee network, the ZGD behaves exactly as if the application
	 * object was implemented on this device.
	 * 
	 * 
	 * @param timeout
	 *          the desired timeout value.
	 ...
	 * @return the endpoint marked as active with the simple descriptor supplied.
	 * @throws IOException
	 *           if an Input Output error occurs.
	 * @throws Exception
	 *           if a general error occurs.
	 ...
	 * by an IPHA in order to exclude an endpoint in the list of active endpoints
	 * and to remove any simple descriptor for this endpoint in the ZDO of the
	 * ZGD. Consequently, if a ZigBee node issues a request to discover the active
	 * endpoints on the ZGD, the ZGD will not indicate the endpoint supplied in
	 * parameter of this procedure in the list of active endpoints. If a ZigBee
	 * node issues a request to discover the simple descriptor supported by this
	 * endpoint on the ZGD, the ZGD will not report any simple descriptor
	 * supported. An IPHA normally call such procedure when a previous
	 * configuration of the ZDO indicated some services supported by this endpoint
	 * for instance due to a prior call to the ConfigureEndpoint procedure.
	 * 
	 * @param endpoint
	 ...
	 * @param duration
	 *          the value expressed in seconds to allow nodes to join the network.
	 ...
	 * procedure is invoked by an IPHA to retrieve the Service Descriptor related
	 ...
	 * IPHA in order to retrieve the list of ZigBee device bindings which are set
	 * on the ZGD.
	 * 
	 * @param timeout
	 *          the desired timeout.
	 ...
	 * @param index
	 *          endpoint on the destination device which is a destination for the
	 *          binding.
	 * 
